...
BEGIN
  ...
  left join (
      select lead_id, property_use, square_footage, number_of_stories, year_built
      from (
          select lead_id, property_use, square_footage, number_of_stories, year_built,
                 row_number() over (partition by lead_id order by updated_at desc nulls last, id desc) as rn
