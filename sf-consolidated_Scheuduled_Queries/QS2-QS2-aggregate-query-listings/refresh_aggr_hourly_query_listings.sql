CREATE OR REPLACE PROCEDURE call_tracking.refresh_aggr_hourly_query_listings(p_days_ago integer)
 LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    CREATE TEMP TABLE staging_hourly_query_dates AS
    ...
    DELETE FROM call_tracking.aggr_hourly_query_listings
    WHERE pst_day IN (SELECT dt FROM staging_hourly_query_dates);

    INSERT INTO call_tracking.aggr_hourly_query_listings (
        pst_hour, pst_day, pst_week, pst_month, pst_quarter, pst_year,
        account_name, service_provider, network, state,
        quote_calls_lead_type_id, source_type_id, marketing_group_id, aid, cid,
    ...
    DROP TABLE staging_hourly_query_dates;
END;
$$
