CREATE OR REPLACE PROCEDURE call_tracking.refresh_aggr_common_query_listings(p_days_ago integer)
 LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    CREATE TEMP TABLE staging_common_query_dates AS
    ...
    DELETE FROM call_tracking.aggr_common_query_listings
    WHERE pst_day IN (SELECT dt FROM staging_common_query_dates);

    INSERT INTO call_tracking.aggr_common_query_listings (
    ...
    DROP TABLE staging_common_query_dates;
END;
$$
