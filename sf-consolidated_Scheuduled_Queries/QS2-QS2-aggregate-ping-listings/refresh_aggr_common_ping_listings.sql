CREATE OR REPLACE PROCEDURE pingpost.refresh_aggr_common_ping_listings(p_days_ago integer)
 LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    CREATE TEMP TABLE staging_common_ping_dates AS
    ...
    DELETE FROM pingpost.aggr_common_ping_listings
    WHERE pst_day IN (SELECT dt FROM staging_common_ping_dates);

    INSERT INTO pingpost.aggr_common_ping_listings (
    ...
    DROP TABLE staging_common_ping_dates;
END;
$$
